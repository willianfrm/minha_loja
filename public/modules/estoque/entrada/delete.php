...
try {
    ...
    $sqlItens = "SELECT * FROM movimentacao_produto WHERE codigo_documento = :doc";
    ...
    foreach ($itens as $item) {
        ...
        if ($doc['tipo'] === 'E') {
            ...
            // Se motivo atualiza custo, verificar se há entradas posteriores
            if ($doc['atualiza_custo']) {
                ...
                if (!$temPosterior) {
                    // Recalcular custo médio e última entrada com base nas entradas anteriores
                    ...
                    if ($anteriores) {
                        ...
                        $novoCustoUlt = $anteriores[0]['valor_unitario']; // última entrada anterior
                    } else {
                        $novoCustoMedio = 0;
                        $novoCustoUlt = 0;
                    }
                ...
                // Motivo não atualiza custo → só estoque
                $sqlUpd = "UPDATE estoque SET estoque=:est WHERE codigo_produto=:prod";
                $stmtUpd = $pdo->prepare($sqlUpd);
                $stmtUpd->execute([
                    ':est' => $novoEstoque,
    ...
    $stmt = $pdo->prepare("DELETE FROM movimentacao_produto WHERE codigo_documento=:doc");
    $stmt->execute([':doc'=>$codigoDoc]);

    // Excluir documento
    $stmt = $pdo->prepare("DELETE FROM documento_movimentacao WHERE codigo=:doc");
    $stmt->execute([':doc'=>$codigoDoc]);

    $pdo->commit();
    echo "<script>alert('Documento estornado com sucesso!'); window.location.href='index.php';</script>";
    exit;

} catch (Exception $e) {
    $pdo->rollBack();
    echo "<script>alert('Erro ao estornar documento: " . $e->getMessage() . "'); window.location.href='index.php';</script>";
    exit;
}
